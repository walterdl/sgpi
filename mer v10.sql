...
CREATE TABLE IF NOT EXISTS `informes_avances` (
  ...
  `revisado` TINYINT(1) NULL DEFAULT 0,
  `detalle_revision` VARCHAR(250) NULL,
  `created_at` DATE NULL,
  `updated_at` DATE NULL,
  `deleted_at` DATE NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_informes_avances_proyectos1_idx` (`id_proyecto` ASC),
  INDEX `fk_informes_avances_formatos_tipos_documentos1_idx` (`id_formato_tipo_documento` ASC),
  CONSTRAINT `fk_informes_avances_proyectos1`
    FOREIGN KEY (`id_proyecto`)
    REFERENCES `proyectos` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_informes_avances_formatos_tipos_documentos1`
    FOREIGN KEY (`id_formato_tipo_documento`)
    REFERENCES `formatos_tipos_documentos` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
