sgpi_app.controller('editar_usuarios_controller', function($scope, $http, Alertify, Upload){
    ...
    $(document).ready(function(){
        ...
        if(usuario == "" && persona ==""){
            ...
            $scope.visibilidad.disable_btn=true;
            alertify.error("No existe el Usuario");
        }else{
            cargarDatos(usuario,persona);  
        }
        
    });
    
    
    function cargarDatos(usuario,persona){
        
        $scope.data.usuarioEditar=jQuery.parseJSON(usuario);
        ...
        else if($scope.data.usuarioEditar.id_rol == 2){//COORDINADOR
            $scope.visibilidad.show_sede = true;
            $scope.visibilidad.show_grupo_inv = true;
            $scope.visibilidad.show_categoria_inv = false;
        }
        else if($scope.data.usuarioEditar.id_rol == 3){//INVESTIGADOR
            $scope.visibilidad.show_sede = true;
            $scope.visibilidad.show_grupo_inv = true;
    ...
    */
    $scope.data = {
        ...
        usuarioEditar_username:null,
        // identificacion_consultada : null,
        // id_readonly: false
    };
    
    $scope.visibilidad = {
        show_datos_usuario:true,
        disable_btn:false,
    ...
    */
    $scope.init = function(data){
        ...
        $scope.visibilidad.show_datos_usuario=false;//desactiva el velo
        ...
        //individual
        $scope.data.tipo_identificacion=$scope.data.usuarioEditar.persona.tipo_identificacion;
        
        if($scope.data.usuarioEditar.persona.sexo == "f"){
            $scope.data.sexo={id: 'f', nombre: 'Mujer'};
        }else{
            $scope.data.sexo={id: 'm', nombre: 'Hombre'};  
        }
        
        $scope.data.formaciones.forEach(function(item) {
            if(item == $scope.data.usuarioEditar.persona.formacion){
                $scope.data.formacion = item;    
            }
        });
        
        $scope.data.rol=$scope.data.usuarioEditar.rol;
        
   
        if($scope.data.usuarioEditar.id_rol != 1){//COORDINADOR
            $scope.data.sede=$scope.data.usuarioEditar.grupo.facultad.sede;
            $scope.cambia_sede();
            $scope.data.grupo_investigacion=$scope.data.usuarioEditar.grupo;
            
        }
        
        if($scope.data.usuarioEditar.id_rol == 3){//INVESTIGADOR
            console.log($scope.data.usuarioEditar.persona.categoria);
            $scope.data.categoria_investigador=$scope.data.usuarioEditar.persona.categoria;
        }
    ...
    */    
    $scope.btn_update_usuario_click = function(){
        ...
        $http({
            ...
            params: {
                identificacion: $scope.data.personaEditar.identificacion,
                username: $scope.data.usuarioEditar.username,
                username_verificar: $scope.data.usuarioEditar_username,
    ...
    */    
    $scope.validar_email = function(){
        if($scope.form_actualizar_usuario.email.$valid){
    ...
    */              
    $scope.validar_username = function(){
        if($scope.form_actualizar_usuario.username.$valid){
